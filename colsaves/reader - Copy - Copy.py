	...
	with open(filename, "rb") as file:
		...
		for i in colonies_dict:
			...
			for x in colonies_dict[i]:
				colony_image = read_image("icons/icons.ss_0.png")
				map_image.paste(colony_image, (colonies_dict[i]['x']*16, colonies_dict[i]['y']*16))
				ImageDraw.Draw(map_image).text((colonies_dict[i]['x']*16, colonies_dict[i]['y']*16), i + '(' + str(len(colonies_dict[i]['colonists'])) + ')',(255,255,255))
